#!/usr/bin/env bash
# Checks Orchestrator Script
# Portable script for running security and quality checks across repositories
...
setup_tools() {
    ...
    if [ -f "${SCRIPT_DIR}/tooling/setup-tools.sh" ]; then
        "${SCRIPT_DIR}/tooling/setup-tools.sh"
        log_success "Tools setup complete"
    else
        log_warning "setup-tools.sh not found, skipping tool setup"
    fi
}
...
update_pinned_actions() {
    ...
    if command -v pinact >/dev/null 2>&1; then
        ...
        log_warning "pinact not available, skipping"
        return 0
    fi
}
...
scan_actions_security() {
    ...
    if command -v zizmor >/dev/null 2>&1; then
        ...
        log_warning "zizmor not available, skipping"
        return 0
    fi
}
...
run_hadolint() {
    ...
    if command -v hadolint >/dev/null 2>&1; then
        ...
        log_warning "hadolint not available, skipping"
        return 0
    fi
}
...
run_quality_gate() {
    ...
    if [ -f "${SCRIPT_DIR}/quality-gate.sh" ]; then
        ...
        log_error "quality-gate.sh not found"
        return 1
    fi
}
...
main() {
    ...
    log_info "Starting Checks Orchestrator"
    ...
    while [ "${iteration}" -lt "${MAX_ITERATIONS}" ]; do
        ...
        if [ ${security_errors} -eq 0 ] && [ ${lint_errors} -eq 0 ] && [ ${precommit_errors} -eq 0 ] && [ ${quality_errors} -eq 0 ]; then
            ...
            if [ "${needs_commit}" = "true" ] && [ "${AUTO_COMMIT}" = "true" ]; then
                ...
                git add -A
                git commit -m "chore: apply checks fixes" || true
                needs_commit=false
            fi
